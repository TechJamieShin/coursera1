...
nycflights %>%
  group_by(origin) %>%
  mutate(ot_dep_rate = sum(dep_type == 'on time')/length(dep_type)) %>%
  arrange(desc(ot_dep_rate))
...
nycflights %>%
  ggplot(aes(x = distance / air_time, y = distance)) +
  geom_point()
...
nycflights %>%
  group_by(dep_type) %>%
  mutate(arr_perc = sum(arr_type == "on time") / n())
```